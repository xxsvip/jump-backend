...
SET search_path TO prod;  -- ← 替换为你的 Schema，如：SET search_path TO sales;
...
DROP SEQUENCE IF EXISTS system_group_seq;
CREATE SEQUENCE system_group_seq
    START 1;
-- ----------------------------
...
(
    id          int8         NOT NULL default nextval('system_group_seq'),
    name        varchar(255)  NOT NULL ,
...
COMMENT ON COLUMN system_group.id IS '编号';
COMMENT ON COLUMN system_group.name IS '名字';
COMMENT ON COLUMN system_group.description IS '描述';
COMMENT ON COLUMN system_group.status IS '状态';
COMMENT ON COLUMN system_group.creator IS '创建者';
COMMENT ON COLUMN system_group.create_time IS '创建时间';
COMMENT ON COLUMN system_group.updater IS '更新者';
COMMENT ON COLUMN system_group.update_time IS '更新时间';
COMMENT ON COLUMN system_group.deleted IS '是否删除';
COMMENT ON COLUMN system_group.tenant_id IS '租户编号';
COMMENT ON TABLE system_group IS '用户组';
